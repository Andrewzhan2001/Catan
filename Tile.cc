...
Tile::Tile(int tileNum)
	: tileNum{ tileNum }, value{ 0 }, geese{ false } {
	if (tileNum == 0) {
		vertex = { 0, 1, 3, 4, 8, 9 };
		edge = { 0, 1, 2, 6, 7, 10 };
	}
...
std::string Tile::getResource() const { return resource; }
...
bool Tile::has(const std::string& type, int index) {
	bool retval = false;
	if (type == "vertex") {
	...
	for (size_t i = 0; i < edge.size(); i++) {
		...
		if (edgeIndex == edge[i]) {
			if (i == 0) {
				...
				adjacentVertex.push_back(vertex[5]);
			}
			else {
...
std::vector<int> Tile::getAdjacentEdge(int vertexIndex) {
	std::vector<int> adjacentEdge;
	for (size_t i = 0; i < vertex.size(); i++) {
		// find the vertex
		if (vertexIndex == vertex[i]) {
...
std::vector<int> Tile::vertexAdjacentVertex(int vertexIndex) {
	std::vector<int> adjacentVertex;
	for (size_t i = 0; i < vertex.size(); i++) {
		// find the vertex
		if (vertexIndex == vertex[i]) {
			if (i == 0) {
				adjacentVertex.push_back(vertex[1]);
				adjacentVertex.push_back(vertex[2]);
			}
			else if (i == 1) {
				adjacentVertex.push_back(vertex[0]);
				adjacentVertex.push_back(vertex[3]);
			}
			else if (i == 4) {
				adjacentVertex.push_back(vertex[2]);
				adjacentVertex.push_back(vertex[5]);
				...
				adjacentVertex.push_back(vertex[i - 2]);
				adjacentVertex.push_back(vertex[i + 2]);
			}
		}
	}
...
vector<string> Tile::printTile(const vector<string>& vertexs, vector<string> edges) {
